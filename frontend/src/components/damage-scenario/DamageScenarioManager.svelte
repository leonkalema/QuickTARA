<script lang="ts">
  ...
  async function handleDeleteScenario(scenario: DamageScenario) {
    if (!confirm(`Are you sure you want to delete the damage scenario "${scenario.name}"?`)) {
      return;
    }
    ...
    try {
      ...
      scenarios = scenarios.filter(s => s.scenario_id !== scenario.scenario_id);
      totalScenarios -= 1;
      updateStats(scenarios);
      
      showSuccess(`Damage scenario "${scenario.name}" deleted successfully`);
    } catch (err) {
      console.error('Error deleting damage scenario:', err);
...
<div class="space-y-6">
  ...
  <!-- Damage Scenario View Dialog -->
  {#if showViewDialog && viewingScenario}
    <div class="fixed inset-0 backdrop-blur-sm bg-neutral-900/40 flex items-center justify-center z-50 p-4 transition-opacity duration-200">
      <div class="w-full max-w-3xl max-h-[90vh] overflow-y-auto bg-white rounded-lg shadow-lg">
        <div class="flex justify-between items-center px-6 py-4 bg-gray-50 border-b border-gray-200">
          <h2 class="text-lg font-medium text-gray-900">Damage Scenario Details</h2>
          <button 
            on:click={handleCloseViewDialog}
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="space-y-4">
            <h3 class="text-md font-medium text-gray-900 border-b pb-2">Component Information</h3>
          ...
          <!-- Metadata -->
          <div class="space-y-4">
            <h3 class="text-md font-medium text-gray-900 border-b pb-2">Metadata</h3>
