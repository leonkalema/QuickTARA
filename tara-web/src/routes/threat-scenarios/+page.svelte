<script lang="ts">
  ...
  let isDeletingAutoGenerated = false;
  async function deleteAllAutoGenerated() {
    if (!$selectedProduct?.scope_id) return;
    if (!confirm('Delete ALL auto-generated threat scenarios (including accepted ones)? This cannot be undone.')) return;
    isDeletingAutoGenerated = true;
    try {
      const result = await scenarioGeneratorApi.deleteAutoThreatScenarios($selectedProduct.scope_id);
      notifications.show(`Deleted ${result.deleted} auto-generated scenarios`, 'success');
      hasExistingAutoDrafts = false;
      await loadData();
    } catch (error: any) {
      notifications.show(error.message || 'Delete failed', 'error');
    } finally {
      isDeletingAutoGenerated = false;
    }
  }
        ...
        {#if canManageScenarios}
          <div class="flex gap-2 flex-wrap">
            ...
            {#if hasExistingAutoDrafts}
              <button on:click={deleteAllAutoGenerated} disabled={isDeletingAutoGenerated}
                class="text-xs font-medium px-3 py-1.5 rounded-md transition-colors disabled:opacity-40"
                style="color: var(--color-error); border: 1px solid var(--color-error-bg);">
                {isDeletingAutoGenerated ? 'Deleting…' : 'Delete all auto-generated'}
              </button>
            {/if}
          </div>
        {/if}
